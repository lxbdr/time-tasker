...
The prisma schema is set up to use PostgreSQL. A `docker-compose.yml` is included that will start a local postgresql
instance.
`npm run dev` will attempt to boot the docker container. Docker (Desktop) must be installed and running.
First run might take a little longer, subsequent starts should be fast.
When done working you can manually stop the services by running `npm run docker:stop`.

Add `DATABASE_URL="postgresql://timetasker:timetasker@localhost:50432/timetasker"` to local .env file
